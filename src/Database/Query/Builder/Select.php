...
class Select extends Where
{
    ...
    {
        $this->_join[] = $this->_lastJoin = new Join($table, $alias, $type);
    ...
    public function union($select, $all = true)
    {
        ...
        if (!$select instanceof self) {
            throw new \Garden\Db\Exceptions\Error('first parameter must be a string or an instance of Database\Query\Builder\Select');
        }
    ...
    public function compile($db = null)
    {
        ...
        $quoteColumn = [$db, 'quoteColumn'];

        // Callback to quote tables
        $quoteTable = [$db, 'quoteTable'];
        ...
        if (empty($this->_select)) {
            ...
            // Select all columns
            $query .= implode(', ', array_unique(array_map($quoteColumn, $this->_select)));
        }

        if (!empty($this->_from)) {
            // Set tables to select from
            $query .= ' FROM ' . implode(', ', array_unique(array_map($quoteTable, $this->_from)));
        }
        ...
        if (!empty($this->_groupBy)) {
            // Add grouping
            $query .= ' ' . $this->_compile_group_by($db, $this->_groupBy);
        }
